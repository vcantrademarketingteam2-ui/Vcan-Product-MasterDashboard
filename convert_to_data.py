...
# leaves blank (e.g. HomePro WMF). Treat a product's presence in these sheets as
# active coverage for that retailer, but only when MASTER didn't already fill it.
COVERAGE_BACKFILL = {
    'HomePro WMF' : 'Homepro',
}
...
# Mirrors the MASTER coverage formula:
#   =SWITCH(XLOOKUP(barcode,'HomePro WMF'!A:A,'HomePro WMF'!M:M,"",0),
#           "ขาย","A","รอขาย","รอขาย","ยกเลิกขาย","ยกเลิกขาย","รอพิจารณา","On process","")
STATUS_MAP = {
    'ขาย'       : 'A',
    'รอขาย'     : 'รอขาย',
    'ยกเลิกขาย' : 'ยกเลิกขาย',
    'รอพิจารณา' : 'On process',
}


def parse_backfill(ws):
    """Return {barcode: coverage_code} from a WMF-channel sheet, mapping its
    Status column exactly like the MASTER XLOOKUP/SWITCH formula."""
    hrow_idx, header = None, []
    for i, row in enumerate(ws.iter_rows(values_only=True, max_col=20)):
        if 'barcode' in [tr(c).lower() for c in row]:
            hrow_idx, header = i, [tr(c) for c in row]
            break
    if hrow_idx is None:
        return {}
    bc_col = find_col(header, 'barcode')
    st_col = find_col(header, 'status')
    out = {}
    for row in ws.iter_rows(min_row=hrow_idx + 2, values_only=True, max_col=20):
        if bc_col is None or bc_col >= len(row):
            continue
        bc = fix_bc(tr(row[bc_col]))
        if not is_bc(bc):
            continue
        st = tr(row[st_col]) if st_col is not None and st_col < len(row) else ''
        code = STATUS_MAP.get(st, '')
        if code:
            out[bc] = code
    return out
...
def main():
    ...
    # 1. MASTER -> products
    print("\n[1/3] Parsing MASTER sheet...")
    products = parse_master(wb['MASTER'])
    print(f"      {len(products)} products found")

    # 2. RETAILER SHEETS -> retailer_data (+ capture coverage backfill barcodes)
    print("\n[2/3] Parsing retailer sheets...")
    retailer_data = {}
    backfill_cov = {}  # retailer_name -> {barcode: coverage_code} to fill blank MASTER cells
    ...
    for sheet_name, retailer_name in SHEET_MAP.items():
        ...
        if sheet_name in COVERAGE_BACKFILL:
            cov_retailer = COVERAGE_BACKFILL[sheet_name]
            backfill_cov.setdefault(cov_retailer, {}).update(parse_backfill(wb[sheet_name]))

    # 2b. Backfill coverage from WMF-channel sheets where MASTER left the cell blank,
    #     using each item's mapped Status (so pending/discontinued carry through too).
    filled = 0
    for p in products:
        for retailer_name, codes in backfill_cov.items():
            code = codes.get(p['barcode'])
            if code and not p['retailers'].get(retailer_name):
                p['retailers'][retailer_name] = code
                filled += 1
    if filled:
        print(f"      backfilled {filled} coverage cell(s) from WMF-channel sheets")
    ...
    print(f"      -> src/retailer_data.js written ({len(retailer_data)} barcodes, {n_entries} retailer entries)")
    print("\nAll done!")
