...
|---------|-------------|
| v3.13.2 | `git pull --rebase` in [update_dashboard.bat](update_dashboard.bat) gains `--autostash`. Caught on the very first real run of the v3.13.1 pipeline: git refuses `pull --rebase` outright when the working tree is dirty (`error: cannot pull with rebase: You have unstaged changes.`) — a modified `HANDOFF.md` was enough. Because that step is followed by `goto fail`, the script would commit the regenerated data and then abort *before* pushing: the exact "commits but never pushes" failure v3.13.1 set out to kill. Verified in a scratch repo with a genuinely divergent remote: without the flag the rebase errors out, with it git autostashes, rebases and reapplies the dirty file unchanged. |
| v3.13.1 | Update pipeline hardened ([update_dashboard.bat](update_dashboard.bat), [copy_packshots.ps1](copy_packshots.ps1)). Bat now: `cd /d "%~dp0"`, a `[0/6]` preflight (py/npm on PATH, `openpyxl`+`PIL` importable, Y: Product Master xlsx reachable, inside a git repo) that fails fast naming the missing item; `errorlevel` checked after **every** step (step 3 PowerShell was previously unchecked, so a missing Y: packshot folder still printed "Done"); stdin fed to the Python steps so their `input("กด Enter...")` no longer stalls the run (`<nul` for convert_to_data, `echo. |` for convert_to_webp whose `input()` is *not* EOFError-guarded); `npm run build` gate before commit; `git add -u` replaced by an explicit pathspec (data js + notification_schedule.json + public/packshots + public/retailers) so HANDOFF.md/WIP never rides along; `git diff --cached --quiet` short-circuits to a clean exit when nothing changed (previously a failed commit fell through into push); `git pull --rebase origin main` before push with conflict instructions; interactive `set /p MSG` prompt removed (optional `update_dashboard.bat "msg"` arg instead) — it broke on `&`/`^`/`"` in the message. ps1: `param([switch]$NoPause)` replaces the broken `-not $args -contains` precedence check, barcode matcher loosened to `^<barcode><anything>` + word-boundary Front/Back (recovers `8857130119162 Vnew Black Pouch Front`, `7322540592412  front` — 3 real packshots that had been silently dropped) with a user-confirmed exclusion list (รวมสคบด้านหลัง named explicitly) for สคบ/ฉลาก label scans and `_dup` copies, and **every skipped filename is now printed** instead of a bare count. Data regenerated: GENERATED_AT was stale at 2026-08-24. |
