...
**NOW line technique (v3.5.0):** NO JS measurement anywhere. Schedule's month gridlines, current-month tint, and NOW line are all rendered by `SchedTrackBg()` (hook-free sub-function) as `left: N%` absolute children inside every `.ps-sched-track` (axis strip + each lane), driven by the `schedMonthPos` memo — the same `filteredSchedRange` percentage math `barPos()` uses for the bars, so drift is structurally impossible. The old `measureSched`/`tlNowX`/`tlInnerRef` `getBoundingClientRect` mechanism is DELETED — do not reintroduce pixel measurement; if the NOW line ever looks off, fix the shared % math, not the DOM.
